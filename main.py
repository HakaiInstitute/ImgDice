...
def main(img_path: str, tile_index: str, out_dir: str):
    ...
    img_saved = [crop_img_to_shp(img, shape, outpath) for shape in sf.shapes()]
...
def crop_img_to_shp(img: rasterio.DatasetReader, shape: shapefile.Shape, out_path: OutPath) -> bool:
    # Get the bbox to crop to
    shp_bbox = BBox(*[round(v) for v in shape.bbox])
    img_bbox = BBox(*list(img.bounds))
